...
pub(crate) fn poker_u8(sample: &Sample, m: i32) -> TestResult {
    ...
    if m == 4 {
        ...
        for chunks in b8[..full_chunks].chunks_exact(8) {
            let x = u64::from_ne_bytes(chunks.try_into().unwrap());
            for i in 0..16{
                ni[((x >> 4*i) & 0x0f) as usize] += 1;
            }
        }
        for x in b8[full_chunks..].iter(){
            ni[(*x & 0x0f) as usize] += 1;
            ni[((*x >> 4) & 0x0f) as usize] += 1;
        }
    } else {
        let full_chunks = b8.len() & (!7);
        for chunks in b8[..full_chunks].chunks_exact(8) {
